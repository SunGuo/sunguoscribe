...
#include "Filter.h"
#include "Syslogmodel.h"     //增加对Syslog协议的增加(可能使用UDP)
...
class Store {
 ...
 protected:
  ...
  /*定义的处理过滤日志的模块(通用模块)   2012_03_29   @author guoxian1*/
  Filter logfilter;
...
/*
* @usage SyslogStore  利用syslog协议向远程syslog写数据（可以使用UDP，TCP协议）
* @author guoxian1 
* @date  2012_04_17
*/

class SyslogStore : public Store {
	public:
		SyslogStore(StoreQueue* storeq,
	...
	protected:
		...
		/*用户使用的黑白名单处理*/
		bool uselistfilter;
		std::string regexfilename;
		bool isblack;
		// state
		bool opened;
		/*处理syslog逻辑的文件协议*/
		boost::shared_ptr<SyslogModel> writesyslog;

	private:
		// disallow copy, assignment, and empty construction
		SyslogStore();
		SyslogStore(SyslogStore& rhs);
		SyslogStore& operator=(SyslogStore& rhs);
};
...
class FileStoreBase : public Store {
 ...
 protected:
  ...
  std::string setHostNameFilename();  //添加函数（guoxian1@staff.sina.com.cn）处理hostname进行处理hostname放到文件名中 2012_01_12


  // Configuration
  std::string baseFilePath;
  std::string hostnameFileName;       //hostname 全局的hostname名称函数
  ...
  bool use_hostname_as_filename;    //添加的字段作为区分不同机器传过来的日志（把字段作为文件名的一部分）。
  /*是否使用前缀 yes/no   */
