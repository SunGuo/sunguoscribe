...
/*
 *@guoxian1 添加 defThresholdBeforeReconnect(NO_THRESHOLD),42 allowableDeltaBeforeReconnect(0)
 */
ConnPool::ConnPool():defThresholdBeforeReconnect(NO_THRESHOLD),
...
bool ConnPool::open(const string& hostname, unsigned long port, int timeout) {
    int msgThreshold = msgThresholdMap.count(ConnPool::makeKey(hostname, port)) ?msgThresholdMap[ConnPool::makeKey(hostname, port)] : defThresholdBeforeReconnect;
	...
	return openCommon(makeKey(hostname, port),
                    shared_ptr<scribeConn>(new scribeConn(hostname, port, timeout,msgThreshold, allowableDeltaBeforeReconnect)));
}

bool ConnPool::open(const string &service, const server_vector_t &servers, int timeout) {
        return openCommon(service,
                    shared_ptr<scribeConn>(new scribeConn(service, servers, timeout,defThresholdBeforeReconnect, allowableDeltaBeforeReconnect)));
}
...
scribeConn::scribeConn(const string& hostname, unsigned long port, int timeout_,int msgThresholdBeforeReconnect_, int allowableDeltaBeforeReconnect_)
  : refCount(1),
  serviceBased(false),
  remoteHost(hostname),
  remotePort(port),
...
scribeConn::scribeConn(const string& service, const server_vector_t &servers, int timeout_,int msgThresholdBeforeReconnect_, int allowableDeltaBeforeReconnect_)
  : refCount(1),
  serviceBased(true),
  serviceName(service),
  serverList(servers),
...
void scribeConn::reopenConnectionIfNeeded() {
    LOG_OPER("ReopenConnection():  threshold %d sent %ld", currThresholdBeforeReconnect, sentSinceLastReconnect);
