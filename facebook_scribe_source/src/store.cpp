...
Store::createStore(StoreQueue* storeq, const string& type,
  ...
  if (0 == type.compare("file")) {
    ...
    return shared_ptr<Store>(new NetworkStore(storeq, category,
                                              multi_category));
  }else if(0 == type.compare("syslog")){
	  return shared_ptr<Store>(new SyslogStore(storeq, category,
                                              multi_category));	
...
SyslogStore::SyslogStore(StoreQueue* storeq,
               const std::string& category,
               bool multi_category)
	:Store(storeq,category,"syslog",multi_category),
	netprotocol("UDP"),
	remoteHost(""),
	remotePort(514),
	uselistfilter(false),
...
void SyslogStore::configure(pStoreConf configuration, pStoreConf parent){
	std::string temp;

	Store::configure(configuration, parent);
	
	/*使用的网络协议  UDP 或者  TCP*/
	configuration->getString("netprotocol",netprotocol);
	/*remote   ip*/
	...
	*处理是否使用前缀的功能
	*/
  if(configuration->getString("not_useprefix",temp)){
  ...
  *是否使用日志过滤功能
  */
  if(configuration->getString("usefilter",temp)){
	if(0 == temp.compare("yes")){
		isusefilter = true;
	} else {
		isusefilter = false;
  ...
  *日志过滤使用的regex
  */
	configuration->getString("log_filter_regex", log_filter_regex);
	LOG_OPER("log_filter_regex  [%s] :",log_filter_regex.c_str());   	
	/*
	*日志过滤使用的配置文件目录
	*/
	if(configuration->getString("regexfilename", regexfilename)){
		uselistfilter=true;
	}else{
		uselistfilter=false;
	}
	...
	*/
	...
	if(configuration->getString("isblack",temp)){
    	if(0 == temp.compare("yes")){
        	isblack = true;
    	} else {
        	isblack = false;
...
void SyslogStore::periodicCheck(){
	
	//nothing do 
}

bool SyslogStore::open(){
	if (isOpen()){
		return (true);
	}
	if (remotePort <= 0 || remoteHost.empty()) {
	...
	if (opened) {
		...
		if(!writesyslog){
			writesyslog = SyslogModel::createSyslogModel(netprotocol, remoteHost, remotePort);
			if(writesyslog->open()){
				opened=true;
			}else{
...
bool SyslogStore::isOpen(){
	return (writesyslog&&writesyslog->isOpen());
}
boost::shared_ptr<Store> SyslogStore::copy(const std::string &category){
	SyslogStore *store = new SyslogStore(storeQueue,category,multiCategory);
	
	shared_ptr<Store> copied = shared_ptr<Store>(store);
	
	store->netprotocol = netprotocol;
	store->remoteHost = remoteHost;
	store->remotePort = remotePort;
	store->not_useprefix = not_useprefix;
	store->isusefilter = isusefilter;
	store->log_filter_regex = log_filter_regex;
	store->uselistfilter = uselistfilter;
	store->regexfilename = regexfilename;
	store->isblack = isblack;	
	return copied;
}

bool SyslogStore::handleMessages(boost::shared_ptr<logentry_vector_t> messages){
	...
	try {
		for (logentry_vector_t::iterator iter = messages->begin();iter != messages->end();++iter) {
			...
			std::string temp_message = (*iter)->message;
			/*如果使用filter进行过滤*/
			if (isusefilter){
				if(!logfilter.filter(temp_message,log_filter_regex)){
					continue;
				}
			}
	  		if(uselistfilter){
				if(!logfilter.listFilter(temp_message,regexfilename,isblack)){
					continue;
				}	
			}
			...
			if(not_useprefix){
				if(!logfilter.removePrefix(temp_message)){
					continue;
				}
			}
	  
      
			if (!writesyslog->write(temp_message)) {
				LOG_OPER("Syslog store failed to write (%lu) messages to file",num_written);
				setStatus("Syslog write error");
				success = false;
				break;
			}
		...
		LOG_OPER(" Syslog store failed to write. Exception: %s", e.what());
		success = false;
  }
	LOG_OPER("Syslog store send <%lu> messages to host <%s> port <%lu> netprotocol <%s>",num_written,remoteHost.c_str(),remotePort,netprotocol.c_str());
  if (!success) {
    close();
...
FileStoreBase::FileStoreBase(StoreQueue* storeq,
  ...
  : Store(storeq, category, type, multi_category),
    ...
    fsType("std"),
    chunkSize(0),
    use_hostname_as_filename(false),
	not_useprefix(false),
	isusefilter(false),
	log_filter_regex(""),
	uselistfilter(false),
...
void FileStoreBase::configure(pStoreConf configuration, pStoreConf parent) {
  ...
  std::string tmp;
  std::string tmp_hostnamefilename;  //@guoxian1 新增临时变量处理新增的参数
  configuration->getString("file_path", baseFilePath);
  configuration->getString("sub_directory", subDirectory);
  configuration->getString("use_hostname_as_filename", tmp_hostnamefilename);  
  if(0 == tmp_hostnamefilename.compare("yes")) {
      use_hostname_as_filename=true;
	  hostnameFileName=setHostNameFilename();
  } 
  else{
	  use_hostname_as_filename=false;
  }
   /*
   *change by guoxian1@staff.sina.com.cn
   *的做法并增加 use_hostname_as_filename 参数进行处理,增加文件名的处理使文件名能够唯一
   */
  ...
  if (configuration->getString("use_tree", tmp)) {
    if (0 == tmp.compare("yes")) {
      ...
      storeTree = true;
	  /*
	  *@guoxian1 注释掉，根据实际情况可能需要分钟回滚，以及注释掉
	  */
      //rollPeriod = ROLL_HOURLY;
    } else {
      storeTree = false;
  ...
  *处理是否使用前缀的功能
  */
  if(configuration->getString("not_useprefix",tmp)){
	if(0 == tmp.compare("yes")){
		not_useprefix = true;
	} else {
		not_useprefix = false;
  ...
  *是否使用日志过滤功能
  */
  if(configuration->getString("usefilter",tmp)){
	if(0 == tmp.compare("yes")){
		isusefilter = true;
	} else {
		isusefilter = false;
  ...
  */
  ...
  if (configuration->getString("write_stats", tmp)) {
    ...
    if(configuration->getString("isblack",tmp)){
        if(0 == tmp.compare("yes")){
            isblack = true;
        } else {
            isblack = false;
...
void FileStoreBase::copyCommon(const FileStoreBase *base) {
  ...
  fsType = base->fsType;
  /*
  *@guoxian1  添加两个参数操作use_hostname_as_filename选项
  */
  use_hostname_as_filename=base->use_hostname_as_filename;
  
  not_useprefix=base->not_useprefix;
  isusefilter=base->isusefilter;
  log_filter_regex=base->log_filter_regex;
  uselistfilter = base->uselistfilter;
  regexfilename = base->regexfilename;
  isblack = base->isblack;

  hostnameFileName=base->hostnameFileName;
...
void FileStoreBase::periodicCheck() {
  /*
  *@guoxian1   (注释三行) 由于采用新的方式进行控制空文件的控制
  *
  */
  //if (!isOpen()) 
  //	{    
  ...
  if (!rotate) {
	...
	/*
	*@guoxian1 注释三行 由于采用新的方式进行控制空文件 2012_01_12 
	*
	*/
 	// if (rotate&&isOpen()) {
 	//     close();
	...
	/*
	*添加 (三行) 新的策略控制空文件 2012_01_12
	*/

	 if(rotate&&currentSize!=0) {
		 rotateFile(rawtime);
	 }
...
string FileStoreBase::makeBaseFilename(struct tm* creation_time) {
  ostringstream filename;
  int minif;
  minif=(creation_time->tm_hour*60+creation_time->tm_min)/5;
  if(minif==0)
  {
   minif=1;
  }
  if (rollPeriod != ROLL_NEVER) {
    if (storeTree) {
      filename << creation_time->tm_year + 1900  << '_'
	       << setw(2) << setfill('0') << creation_time->tm_mon + 1 << '_'
	       << setw(2) << setfill('0') << creation_time->tm_mday << '/'
	       << setw(2) << setfill('0') << creation_time->tm_hour << '/';
      filename << baseFileName;
	  if(use_hostname_as_filename){
			filename<<"-"<<hostnameFileName;
	  }
      filename << '-' << creation_time->tm_year + 1900  << '_'
             << setw(2) << setfill('0') << creation_time->tm_mon + 1 << '_'
              << setw(2) << setfill('0')  << creation_time->tm_mday << "_"
	       << setw(2) << setfill('0')  << creation_time->tm_hour<<"-"
		   <<creation_time->tm_year+1900<<setw(2)
		   <<setfill('0')<<creation_time->tm_mon + 1
		   <<setw(2)<<setfill('0')<<creation_time->tm_mday
		   <<setw(3)<<setfill('0')<<minif;
    } else {
      filename << baseFileName;
...
/*
*add by guoxian1@staff.sina.com.cn
*处理hostname放到文件名中的函数，在添加 pid
*
*/

std::string FileStoreBase::setHostNameFilename(){
	char hostname[255];
	std::stringstream str_pid;
    str_pid<<getpid();          //得到进程的pid
	std::string hostname_pid;
	int error=gethostname(hostname,sizeof(hostname));
	if(error){
		LOG_OPER("[%s] WARNING: gethostname returned error: %d",categoryHandled.c_str(), error);
	}
	string hoststring(hostname);
	if (hoststring.empty()){
		LOG_OPER("[%s] WARNING: could not get host name",categoryHandled.c_str());
		hoststring="hostname";
		hostname_pid=hoststring.append("_").append(str_pid.str());
	}else{
  		hostname_pid=hoststring.append("_").append(str_pid.str());
	}
	return hostname_pid;
...
bool FileStore::openInternal(bool incrementFilename, struct tm* current_time) {
  ...
  try {
    ...
    success = writeFile->createDirectory(baseFilePath);
		...
		/*
		*@guoxian1  ，添加控制scribe产生本地路径(通过use_tree)进行对本地路径的控制
		*/
		if(storeTree){
			std::string file_directory=file.substr(0,file.find_last_of("/"));
			success=writeFile->createDirectory(file_directory);
		}
  ...
  try {
    for (logentry_vector_t::iterator iter = messages->begin();
	 ...
	 /*处理黑白名单的逻辑*/
      ...
      //unsigned long message_length = (*iter)->message.length();// 2012_03_31 @guoxian1 注释 for filter
      ...
      write_buffer += frame;
      //write_buffer += (*iter)->message;// 2012_03_31 @guoxian1 注释 for filter
	...
	if ((write_buffer!=""||write_buffer.length()>0)&&success==true) {
        if (!write_file->write(write_buffer)) {
          LOG_OPER("[%s] File store failed to write (%lu) messages to file",
                   categoryHandled.c_str(), messages->size());
          setStatus("File write error");
...
bool ThriftFileStore::handleMessages(boost::shared_ptr<logentry_vector_t> messages) {
  ...
  for (logentry_vector_t::iterator iter = messages->begin();
	...
	*/ 
	...
	uint32_t length = temp_message.size();
	
    try {
      thriftFileTransport->write(reinterpret_cast<const uint8_t*>(temp_message.data()), length);
      currentSize += length;
      ++eventsWritten;
      ++messages_handled;
  ...
  if (serviceBased) {
    ...
    if (useConnPool) {
      ...
      unpooledConn = shared_ptr<scribeConn>(new scribeConn(serviceName,
            servers, static_cast<int>(timeout),defThresholdBeforeReconnect, allowableDeltaBeforeReconnect));
    ...
    if (useConnPool) {
     ...
     int msgThreshold = msgThresholdMap.count(ConnPool::makeKey(remoteHost, remotePort)) ?msgThresholdMap[ConnPool::makeKey(remoteHost, remotePort)]: defThresholdBeforeReconnect;
      unpooledConn = shared_ptr<scribeConn>(new scribeConn(remoteHost,remotePort, static_cast<int>(timeout),msgThreshold, allowableDeltaBeforeReconnect));
      opened = unpooledConn->open();
      if (!opened) {
        unpooledConn.reset();
      }
...
NetworkStore::handleMessages(boost::shared_ptr<logentry_vector_t> messages) {
  ...
  if (!isOpen()) {
   ...
   boost::shared_ptr<logentry_vector_t> temp_message_v(new logentry_vector_t);
   ...
   for (logentry_vector_t::iterator iter = messages->begin();
	 ...
	 if(uselistfilter){
	  ...
	  logentry_ptr_t t_entry = logentry_ptr_t(new LogEntry);
	  t_entry->category =(*iter)->category;
	  t_entry->message =temp_message;
	  temp_message_v->push_back(t_entry);
	}
	
	
  bool tryDummySend = shouldSendDummy(temp_message_v);
  ...
  if (useConnPool) {
    if (serviceBased) {
      if (!tryDummySend ||
        ...
        ret = g_connPool.send(serviceName, temp_message_v);
      }
    } else {
      if (!tryDummySend ||
          (ret = g_connPool.send(remoteHost, remotePort, dummymessages)) ==
          CONN_OK) {
        ret = g_connPool.send(remoteHost, remotePort, temp_message_v);
      }
    }
  } else if (unpooledConn) {
    if (!tryDummySend ||
        ((ret = unpooledConn->send(dummymessages)) == CONN_OK)) {
      ret = unpooledConn->send(temp_message_v);
    }
  } else {
    ret = CONN_FATAL;
